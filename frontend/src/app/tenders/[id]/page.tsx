...
import {
  ...
  ShoppingCart, Search, Tag, BookOpen, RefreshCw, ChevronDown, ChevronUp
} from 'lucide-react';
...
function PdfViewer({ proxyUrl }: { proxyUrl: string }) {
  ...
  const handleOpen = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (!res.ok) { win?.close(); setFailed(true); return; }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      if (win) win.location.href = url;
    } catch { win?.close(); setFailed(true); }
  };
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    setFailed(false);
    try {
      const res = await fetch(`${proxyUrl}?download=1`);
      if (!res.ok) { setFailed(true); return; }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href = url; a.download = 'techspec.pdf'; a.click();
    ...
    <div className="flex flex-col gap-1 pt-1">
      <div className="flex items-center gap-2">
        <button onClick={handleOpen}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-700 bg-gray-800 hover:bg-gray-700 text-xs text-gray-200 transition-colors">
      ...
      {failed && (
        <p className="text-[10px] text-amber-400">PDF-документ не найден в тендере. Нажмите «Полный анализ» для повторного поиска.</p>
      )}
    </div>
  );
}

// Key fields to highlight at the top of the spec table
const SPEC_KEY_FIELDS = [
  'наименование лота', 'лоттың атауы',
  'описание лота', 'лоттың сипаттауы',
  'дополнительное описание', 'қысқаша сипаттауы',
  'количество', 'саны',
  'единица измерения', 'өлшем бірлігі',
  'срок поставки', 'жеткізу мерзімі',
  'место поставки', 'места поставки', 'жеткізу орны',
  'описание и требуемые', 'сипаттамасы',
];

// Kazakh-specific letters absent from Russian alphabet
const _KZ_RE = /[әғқңөұүһіӘҒҚҢӨҰҮҺІ]/;
const _isKazakh = (s: string) => _KZ_RE.test(s);

// Lines/keys that belong to bank-guarantee boilerplate — filter entirely
const _GUARANTEE_LINE_RE = /обеспечени[ея]|банковск|наименование\s+банк|реквизит|организатор[а]?\s+государствен|гарантийн|гарантодател|бенефициар|принципал|сумма\s+(в\s+цифрах|в\s+словах|гарантии)|срок\s+действия\s+гарантии|настоящ.{0,10}гарантией|вступает\s+в\s+силу|исполнение\s+обязательств|конкурсная\s+документация|\[документ\]\s*:\s*обеспечени/i;

function _filterSpecText(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Skip Kazakh lines
    if (_isKazakh(trimmed)) continue;

    // Skip [ОПИСАНИЕ ЛОТА] block if it contains boilerplate — the whole block to next blank line
    if (trimmed.startsWith('[ОПИСАНИЕ ЛОТА]')) {
      skipSection = true;
      continue;
    }

    // Detect a guarantee section header (standalone line matching guarantee pattern)
    if (trimmed && trimmed.length < 100 && _GUARANTEE_LINE_RE.test(trimmed)) {
      skipSection = true;
      continue;
    }

    // Blank line — check if next non-blank line starts a new real section
    if (!trimmed) {
      // Peek ahead to see if we're leaving boilerplate
      const nextNonBlank = lines.slice(i + 1).find(l => l.trim());
      if (nextNonBlank && !_GUARANTEE_LINE_RE.test(nextNonBlank.trim()) && !_isKazakh(nextNonBlank.trim())) {
        skipSection = false;
      }
...
function parseSpecRows(text: string): { key: string; value: string }[] | null {
  ...
  for (const line of lines) {
    ...
    if (colonIdx > 0 && colonIdx < 200) {
      ...
      if (_isKazakh(key)) continue;
      if (_GUARANTEE_LINE_RE.test(key)) continue;
      rows.push({ key, value });
    }
  }
...
function ProductCard({ resolved, analysis, characteristics: charsProp, specText, rawSpecText, techspecPdfUrl, lotId, documents = [], tenderDocuments = [], tender, platform, onDownloadDocument, onOpenDocument }: {
  resolved:        ResolvedProduct | null;
  analysis:        any;
  characteristics: string | null;
  specText?:       string | null;
  rawSpecText?:    string | null;
  techspecPdfUrl?: string | null;
  lotId?:          string;
  documents?:      any[];
  tenderDocuments?: any[];
  tender?:         any;
  ...
  return (
    <div className="card">
      ...
      {specDocuments.length > 0 ? (
        ...
        <div className="rounded-lg border border-gray-700 bg-gray-800/40 px-4 py-3 mb-3">
          <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Документы закупки</p>
          <p className="text-xs text-gray-400 mt-0.5">PDF не прикреплён. Откройте исходную закупку или проверьте ниже извлечённый текст.</p>
          {hiddenGuaranteeCount > 0 && (
            <p className="text-xs text-yellow-400 mt-1.5">
              Скрыто {hiddenGuaranteeCount} документ{hiddenGuaranteeCount === 1 ? '' : 'а'} обеспечения заявки.
            </p>
          )}
        </div>
      )}
        ...
        <div className="mb-3">
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {productLinks.map((link: any, i: number) => {
              ...
              const hasPrice = link.price_rub > 0 || link.price_kzt > 0;
              const score: number | null = link.relevance_score ?? null;
              const scoreColor = score === null ? '' : score >= 70 ? 'text-green-400' : score >= 50 ? 'text-amber-400' : 'text-red-400';
              const scoreBg    = score === null ? '' : score >= 70 ? 'bg-green-400/10 border-green-400/30' : score >= 50 ? 'bg-amber-400/10 border-amber-400/30' : 'bg-red-400/10 border-red-400/30';
              return (
                <a
                  key={i}
                  href={link.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  title={link.match_reason || undefined}
                ...
                >
                  <div className="min-w-0 flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      {link.match_reason && (
                        <p className="text-[10px] text-gray-500 truncate mt-0.5">{link.match_reason}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {score !== null && (
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${scoreColor} ${scoreBg}`}>
                        {score}%
                      </span>
                    )}
                    {link.price_kzt > 0 && (
                      <span className="text-xs font-semibold text-green-400">{link.price_kzt.toLocaleString('ru')} ₸</span>
                    )}
                    {link.price_rub > 0 && (
                      <span className="text-xs font-semibold text-gray-300">{link.price_rub.toLocaleString('ru')} ₽</span>
                    )}
                    {!hasPrice && (
    ...
    <div className="p-6 max-w-5xl">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <ProductCard
          ...
          specText={(lot as any).technical_spec_text ?? null}
          rawSpecText={(lot as any).raw_spec_text ?? null}
          techspecPdfUrl={(lot as any).techspec_pdf_url ?? null}
          lotId={id as string}
